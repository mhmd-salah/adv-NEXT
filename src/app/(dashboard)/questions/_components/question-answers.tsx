...
export default function QuestionAnswers({ activeQuestion }: { activeQuestion: number }) {
    ...
    <div className='flex flex-col'>
      <div className='flex justify-between bg-zinc-400 items-center p-4'>
        ...
        <Button
          ...
          onClick={() => setIsAddingAnswer(true)}>Add Answer</Button>
      </div>

      {/* Answers */}
      <ul className='flex flex-col gap-2 border p-2'>
        {answers.map((answer, i) => <li key={answer.id} className='flex bg-zinc-700'>
          {/* Answer Text */}
          ...
          <Button
            size='sm'
            onClick={() => {
              form.setValue(`questions.${activeQuestion}.answers.${i}.isCorrect`, !answer.isCorrect)
            }}
            variant={answer.isCorrect ? 'ghost' : 'default'}
          >{answer.isCorrect ? 'Correct Answer' : 'Mark Correct'}</Button>
        </li>)}
      </ul>

      {isAddingAnswer && <div className='flex bg-zinc-500 p-1'>
        <Input
